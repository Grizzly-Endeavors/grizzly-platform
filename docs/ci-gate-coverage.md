...
- **tsc `--noEmit` + full `strict` family** — strict null checks and complete type strictness. By default this runs against the gate's `tsconfig.base.json`. When a node project sets `tsconfig` in its `gate-config.json`, the harness instead wraps the repo's own tsconfig: a generated config `extends` it (inheriting the repo's `paths`/`baseUrl`/module resolution) while force-overriding the entire `strict` compiler family locally — so the typecheck is *project-aware* (it understands the repo's path mapping) **and** the repo cannot opt out of any strictness sub-flag (`extends` is overridden per-key, and tsc's default `include` means the repo can't shrink the typechecked set either). See [ADR-029](decisions/029-gate-config-honest-map.md).

⚠️ **Remaining limitation:** eslint here only lints `.js/.mjs/.cjs` (TypeScript files are type-checked by tsc, not linted by eslint). Deep *type-aware linting* (the JS analog of clippy pedantic) needs `typescript-eslint` and is still deferred. tsc is now project-aware when a repo declares its `tsconfig` (above), so the old "strict-but-not-project-aware typecheck" caveat no longer applies — but treat Node coverage as a strong baseline, not full parity with the Rust/Python regimes (no type-aware lint pass).
