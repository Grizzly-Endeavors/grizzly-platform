...
2. **Delivery via `blueprints.configMaps`.** Blueprint YAML lives as real files under `kubernetes/infrastructure/authentik/blueprints/`, turned into the `authentik-blueprints` ConfigMap by a kustomize `configMapGenerator`. The HelmRelease lists that ConfigMap under `values.blueprints.configMaps`; the chart mounts its `.yaml` keys into **server and worker** at `/blueprints/custom/`. No manual `global.volumes`/`volumeMounts` plumbing.
