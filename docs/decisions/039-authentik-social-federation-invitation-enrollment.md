...
**Federate credentials to social providers (Discord, GitHub, Google) and gate account creation behind admin-issued invitations.** No user objects are pre-provisioned.

1. **Federated credentials.** Three `authentik_sources_oauth.oauthsource` entries (Discord, GitHub, Google). Authentik stores no password for these users; the provider vouches for identity at login. Client id/secret follow the ADR-037 secret path (`!Env` ← `global.env` ← ESO-synced `authentik-secrets` ← OpenBao `secret/grizzly-platform/platform/authentik`). Sources use `user_matching_mode: email_link` so a person who logs in with both Discord and GitHub resolves to one account (the second provider links to the existing user by verified email) rather than creating duplicates — safe because both providers supply verified emails and enrollment is invitation-gated.
...
- **Google scopes.** Google uses only `openid`/`email`/`profile` (non-sensitive), so no Google app verification is required.
