...
- **Humidity/condensation becomes a real risk — mitigated by the existing garage dehumidifier.** Active ventilation pulling humid summer air across cool below-grade surfaces is a classic condensation setup. The garage already runs a **dedicated dehumidifier drained by hose to the outside**, which holds the space at **~43% RH year-round including Georgia summers** (it only struggles when the garage doors are left open, which is rare). The operator will add a **closet-specific dehumidifier** if the enclosed rack space proves drier-resistant. Humidity/temp monitoring still lands in the observability stack as a first-class signal (sensor + alert threshold per the readiness checklist) — as verification that the mitigation is holding, not as an open question.
- **Water ingress is a risk — mitigated by off-slab shelving.** Below-grade rooms flood/seep. **Sturdy shelving is already in place** and the gear (including the UPS/PDU) sits on it, **off the slab**. A water/leak sensor near the floor still lands in the observability stack as an early-warning signal.
