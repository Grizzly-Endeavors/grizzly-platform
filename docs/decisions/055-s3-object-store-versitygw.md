...
Replace both MinIO instances with **versitygw** (Versity S3 Gateway) — a stateless S3 gateway that stores objects as plain files at human-readable POSIX paths on the *existing* storage tiers, retaining the [ADR-012](012-hot-services-on-zfs-minio-split.md) obs/bulk split (obs on ZFS, bulk on MergerFS+SnapRAID). Durability is delegated to ZFS and SnapRAID (the gateway holds no authoritative state); IAM is backed by OpenBao (versitygw's Vault IAM mode). This is **provisional pending a live spike** — commit only after validation succeeds.
...
- **Two instances retained** (obs/bulk) — same operational multiplicity as today; the tiering rationale in [ADR-012](012-hot-services-on-zfs-minio-split.md) is unchanged.
