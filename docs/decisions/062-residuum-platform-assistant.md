...
- **Mutation is IaC-only, through pull requests.** The agent edits repo files, commits, pushes a branch, and opens a PR; a human merges and Flux/CI applies. Its GitHub token is org-wide but scoped to **Contents + Pull requests** only — no merge, no admin — so branch protection is a real gate rather than a convention.
...
- The assistant can *propose* infrastructure changes but never apply them directly. Real capability growth is a deliberate, reviewable step (widen the token, or grant a direct credential), not a drift.
...
- **Give the agent direct write credentials** (`kubectl apply`, `bao write`, push to default branches). Rejected for now: PR-only keeps a human in the loop while trust is established, and matches the platform's IaC rule that changes land through version control.
