...
- **Dependencies, in order of blast radius:** foundation Postgres (Vikunja will not start without it), Authentik (no new sign-ins), s3-bulk (attachment upload and download fail, everything else works), Stalwart (reminder mail stops). First step when it misbehaves: `kubectl -n vikunja logs deploy/vikunja`, then `/app/vikunja/vikunja doctor` inside the pod, which checks the database, S3 file storage, mailer and OIDC provider. Its ParadeDB line always fails here and can be ignored: it looks up `public.pg_extension` rather than `pg_catalog.pg_extension`, and ParadeDB is an optional search backend Vikunja is not configured to use.
