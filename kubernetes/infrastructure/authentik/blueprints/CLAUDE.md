...
Access is **closed, gated by invitation**. The flow `grizzly-invite-enrollment` is gated by the `grizzly-invite-gate` **expression policy** (a flow-level `authentik_policies.policybinding`): it reads the `grizzly_invite` cookie planted by the [grizzly-invite broker](https://github.com/Grizzly-Endeavors/grizzly-invite) and POSTs it to the broker's `/verify`; a missing/invalid/used invitation (or a broker error) denies the flow, so an uninvited social login creates no account. The cookie is the bridge that survives the OAuth round-trip Authentik flow context does not — which is why the old paste-a-code prompt + invitation stage were removed. Enrolled users are auto-added to `grizzly-users` by the User Write stage (`create_users_group`), created active. See [ADR-040](../../../../docs/decisions/040-invite-broker-cookie-bridged-enrollment.md) (and [ADR-039](../../../../docs/decisions/039-authentik-social-federation-invitation-enrollment.md) for the original invitation model).

**Onboarding a person** (per-user cost = one invite + a message): mint an invite in the broker (`POST /api/invites` with the admin bearer token) and send them the returned `invite.bearflinn.com/i/<token>` link. They click it, sign in with **Discord**, **GitHub**, or **Google**, and are enrolled — no code to paste. To pre-scope the person into one or more groups (e.g. a household or friend circle from `groups.yaml`), pass `{"groups": ["Big Bear Hollow"]}` in the mint body; the enrollment gate adds them on top of `grizzly-users` (unknown names are ignored). Promotion to `grizzly-admins` is a separate, deliberate step (not automated). Revoking access is deleting the user (and their source connection) in Authentik; revoking an *unused* invite is `DELETE /api/invites/<id>` on the broker.
